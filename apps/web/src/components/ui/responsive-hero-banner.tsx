    ...
    <section className="relative isolate min-h-screen w-full overflow-hidden">
      ...
      <header className="relative z-10 xl:top-4">
        <div className="mx-6">
          ...
          {mobileMenuOpen && (
            <div className="mt-3 flex flex-col gap-1 rounded-2xl bg-white/5 p-3 ring-1 ring-white/10 backdrop-blur md:hidden">
              {navLinks.map((link, index) => (
                <a
                  key={index}
                  href={link.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`rounded-xl px-3 py-2.5 font-sans text-sm font-medium transition-colors hover:bg-white/10 hover:text-white ${
                    link.isActive ? 'text-white/90' : 'text-white/80'
                  }`}
                >
                  {link.label}
              ...
              <a
                href={ctaButtonHref}
                onClick={() => setMobileMenuOpen(false)}
                className="mt-1 inline-flex items-center justify-center gap-2 rounded-xl bg-white px-3.5 py-2.5 font-sans text-sm font-medium text-neutral-900 transition-colors hover:bg-white/90"
              >
                {ctaButtonText}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="relative z-10">
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-16 sm:pt-28 md:pt-32 lg:pt-40">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="animate-fade-slide-in-2 font-sans text-4xl leading-tight font-normal tracking-tight text-white sm:text-5xl md:text-6xl lg:text-7xl">
              {title}
